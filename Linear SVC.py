...
def get_parametrii_optimi(dict_parametrii, feat_train, label_train):
    ...
    grd = GridSearchCV(LinearSVC(), dict_parametrii,n_jobs = -1)
...
vectorizer_text1 = CountVectorizer()
feat_train_text1 = vectorizer_text1.fit_transform(df_antrenare_preprocesat['sentence1'])
feat_test_text1 = vectorizer_text1.transform(df_validare_preprocesat['sentence1']) # applying the features set to the dataset as separate vectorizers

vectorizer_text2 = CountVectorizer()
...
dict = get_parametrii_optimi(parameters_svm, features_train, df_antrenare_preprocesat['label'])

clasificator = LinearSVC(multi_class = 'ovr', max_iter = 2000, C = 0.1)
